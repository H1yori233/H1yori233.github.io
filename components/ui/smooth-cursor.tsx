  ...
  useEffect(() => {
    ...
    const checkHoverState = (x: number, y: number) => {
      ...
      const isClickable = (el: Element | null): boolean => {
        ...
        // Check common clickable elements and attributes
        const clickableElements = ['A', 'BUTTON', 'INPUT', 'SELECT', 'TEXTAREA'];
        const hasClickableStyle = window.getComputedStyle(el).cursor === 'pointer';
        const hasClickEvents = el.hasAttribute('onclick') || 
    ...
    const smoothMouseMove = (e: MouseEvent) => {
      ...
      // Check hover state
      const hovering = checkHoverState(currentPos.x, currentPos.y);
      if (hovering !== isHovering) {
        setIsHovering(hovering);
        // Scale up when hovering clickable elements
        scale.set(hovering ? 1.5 : 1);
      }
    ...
    // Handle click events specially to ensure precise cursor placement
    const handleClick = () => {
      if (isHovering) {
        // Add small animation to indicate click
        scale.set(1.2);
        setTimeout(() => {
